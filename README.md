<div align="center">
  <h1>🎮 osu! Cheat List 🎮</h1>
  <p><em>A directory of osu! cheat clients & hacks — where to find them and what they do.</em></p>
  <p><em>Updated 2026-05-17</em></p>
</div>

> ⚠️ Cheats are detectable on Bancho and most servers. Use at your own risk.

## Table of Contents
- [Kawata/Aeris](#kawata-aeris)
- [Maple](#maple)
- [Arc](#arc)
- [osu!rx](#osu-rx)
- [Patched osu! b20220424](#patched-osu)
- [📚 Legacy / Archive](#legacy-archive)
  - [AQN](#aqn)
  - [Abypass](#abypass)
  - [Skooter](#skooter)
  - [Ainu](#ainu)

---

## 🎯 Kawata/Aeris <a name="kawata-aeris"></a>
_Originally developed by **Panini Céleste**. Currently maintained by **TheFantasticLoki**, with contributions from Maple Syrup and Chewy/Pythr._

**Download:** https://storage.kawata.pw/get/osu!Kawata.zip *(public beta)*  
**Virus analysis:** https://www.hybrid-analysis.com/sample/3a08fea940bb7028b08b0a6688cae86344af3fc5ea2340ff03a29d95be090614

> 💡 Add the install folder as an antivirus exclusion to prevent issues.

**Cheats:** Aim Correction (improved Skooter AC, optional Tap-on-Correct for non-RX, relative range support) · Relax Hack (SkooterRX) · Timewarp (Rate & Multiplier) · Automatic CS Changer · AR Changer · HD Remover · FL Remover

**Beta:** Maple Aim Assist (V1 + V2 submitting; V3 and L-V1 not submitting) · Dedicated input thread (aim only — 1/2/4/8/16K FPS options)

---

## 🎯 Maple <a name="maple"></a><a name="maple-client"></a>
_**Paid** hack by **Maple Syrup**. Constantly updated. No built-in limits._

**Download:** https://maple.software/  
**Discord:** https://discord.com/invite/P9Yn8FfC2v

**Cheats:** Aim Assist (3 versions) · Relax Hack · Timewarp · AR Changer · FL Remover · HD Remover · CS Changer

---

## 🎯 Arc <a name="arc"></a>
_Developed by **Aochi**._

**Download:** https://m1.aochi.uk/r/osu!.exe

> ⚠️ Do not put `osu!.exe` in your main osu install — make a new folder for the Arc client.

**Cheats:** Maple Aim Assist (V1–V3) · Aim Correction · Relax Hack · Timewarp · AR Changer · CS Changer · HD Remover · FL Remover

---

## 🎯 osu!rx <a name="osu-rx"></a>
_Free hack by **mrflashstudio**, updated by **Sasuke**._

**Download:** https://www.mpgh.net/forum/showthread.php?t=1538659

**Setup:**
1. osu!rx does **not** run on the latest osu!. Download [b20220424](https://osekai.net/snapshots/versions/b20220424/b20220424.zip).
2. Apply a TLS-patched osu.exe — the [original Discord CDN mirror](https://cdn.discordapp.com/attachments/598976475579809860/1082594775988981760/osu.exe) is now inactive; ask MPGH or the osu!rx Discord for a current source.
3. Create a `_STAGING` file in your osu! folder to block auto-updates.

---

## 📚 Legacy / Archive <a name="legacy-archive"></a>
_Discontinued or no-longer-maintained clients. Kept here for historical reference — most downloads will be broken or unsafe by now._

### 🎯 AQN <a name="aqn"></a>
_By **Rumoi**. Now free, no longer maintained. **Crashes on the latest osu! version.**_

- Website (legacy): https://theaquila.net/
- No-login / free version: https://github.com/rumoi/AQN_nologin

### 🎯 Abypass <a name="abypass"></a>
_Successor to Skoot.er by **Chewy/Pythr**, previously maintained by Aochi. **Mostly discontinued — downloads have been lost.**_

- [Main link](https://abypass.fumo.lol/updater) *(unverified)*
- [Mirror 1](https://cute.cat-girls.club/u/logytbz.zip)
- [Discord](https://discord.gg/RFj2839kbw)

**Cheats:** Aim Correction · HD Remover · CTB Relax Hack (control catcher with mouse) · Automatic CS Changer · Relax Hack · FL Remover · Timewarp · AR Changer

**Other:** In-game PP counter · Built-in updater · Server switcher

### 🎯 Skooter <a name="skooter"></a>
_By **Aoba Suzukaze, VacCat, Chewy/Pythr**. No longer maintained — replaced by Abypass._

- [Main link](https://skooter.shibe.lol/)
- [Patched build](https://cdn.discordapp.com/attachments/598976475579809860/1082588578858680330/skooter_b5.exe) *(for increased Aim Correction)*
- [skooter_new](https://cdn.discordapp.com/attachments/883824772021108736/1083581690284359841/skooternew.zip) *if Skooter won't open*

### 🎯 Ainu <a name="ainu"></a>
_By **Aoba Suzukaze**, edited by **Chewy/Pythr**. No longer maintained — replaced by Skooter, then Abypass._

> ⚠️ Only download from trusted sources (MPGH, the Skoot.er Discord, or the Ainu Cheaters Discord). Copies from elsewhere are known to contain unsafe modifications.

- [Main link](https://cdn.discordapp.com/attachments/827128975897657344/889117178744434738/ainu-cheat.exe)
- [Alternate version](https://cdn.discordapp.com/attachments/837034085478039574/902318926980071434/ainu-cheat_1.exe)

---

## 📦 Patched osu! b20220424 <a name="patched-osu"></a><a name="patched-osu-20220424"></a>
_Base client for osu!rx and other legacy hacks._

- [Main link](https://osekai.net/snapshots/versions/b20220424/b20220424.zip)
- ~~[Alternate link](https://cdn.discordapp.com/attachments/598976475579809860/1082594775988981760/osu.exe)~~ *(Discord CDN link inactive)*
